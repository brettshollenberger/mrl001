    ...
    .factory('CommonInterface', function($location, FormHelper, $rootScope) {
        return {
            save: function(options) {
                ...
                if (isArray(form))   { processForms(); }
                if (isObj(form))     { processForm();  }
                // ********** Private Methods ************ //
                ...
                function processForms() {
                    form.forEach(function(f) { checkFormValidity(f); });
                    if (formsValid)          { successCallback(); }
                    if (!formsValid)         { form.forEach(function(f) { markInvalidFields(f); }); }
                }

                function checkFormValidity(form) {
                    if (form.$invalid) { formsValid = false; }
                }

                function processForm() {
                    if (form.$valid)   { successCallback(); }
                    if (form.$invalid) { markInvalidFields(form); }
                }
